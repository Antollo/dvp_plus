...
from lcmr.utils.elliptic_fourier_descriptors import compute_rotational_symmetry, normalize_efd, reconstruct_contour
...
class Scene:
    ...
    def from_tensors_sparse(
        ...
        order: TensorType[batch_dim, layer_dim, object_dim, 1, torch.float32] = None,
        ...
        scene = Scene(
            ...
            layer=Layer(
                ...
                object=Object(
                    ...
                    transformation=LazyAffine.from_tensors(translation=translation, scale=scale, angle=angle, rotation_vec=rotation_vec, single_scale=single_scale),
                    appearance=Appearance(batch_size=[batch_len, layer_len, object_len], confidence=confidence, order=order if order!=None else confidence, color=color),
                    efd=efd,
    ...
    @staticmethod
    @typechecked
    def dist(scene_true: "Scene", scene_pred: "Scene", fields: str = "tsrceb", aggregate: bool = True, n_objects: int = 4) -> Union[torch.Tensor, list[torch.Tensor]]:
        total = [torch.zeros([1], device=scene_true.device) for _ in range((len(fields) + 1))]
        for n in reversed(range(1, n_objects + 1)):
            mask = scene_true.layer.object.appearance.confidence.round().sum(dim=-2).flatten() == n

            if mask.sum() > 0:
                scene_true_n = scene_true[mask].clone()
                
                # TODO: move sorting out of the loop?
                sorted_idx = scene_true_n.layer.object.appearance.confidence.argsort(dim=-2, descending=True)[..., 0]
                scene_true_n.layer.object = torch.gather(scene_true_n.layer.object, dim=-1, index=sorted_idx)[..., :n]
                scene_pred_n = scene_pred[mask]
                
                assert (scene_true_n.layer.object.appearance.confidence.round().sum(dim=-2).flatten() == n).all()

                new_total = Scene.dist_internal(scene_true_n, scene_pred_n, fields=fields)

                for i in range(len(total)):
                    total[i] = total[i] + new_total[i] * (mask.sum() / scene_true.shape[0] * sqrt(n) / sqrt((n_objects + 1) / 2))
    ...
    @staticmethod
    @typechecked
    def dist_internal(scene_true: "Scene", scene_pred: "Scene", fields: str) -> Union[torch.Tensor, list[torch.Tensor]]:
        a = scene_true
        b = scene_pred
