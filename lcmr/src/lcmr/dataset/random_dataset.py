    ...
    with torch.no_grad():
        ...
        for _ in range(ceil(options.n_samples / batch_size)):
            ...
            scene = Scene.from_tensors_sparse(
                ...
                confidence=to_unary(n_visible, n_objects)[..., None],
        ...
        images = [renderer.render(scene).image[..., :3].cpu() for scene in scene_iterator] if options.return_images else [None] * len(scenes)
