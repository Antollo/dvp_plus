...
# 1 TODO: remove the for loop, process "in batch", tensor of all masks will be required later to count ARI for multiple objects!
# 2 TODO: Handle masks in a different way
...
def simple_flat_rgba_blend(colors: torch.Tensor, fragments: Fragments, blend_params: BlendParams, return_alpha: bool) -> Sequence[torch.Tensor]:
    ...
    prob_map, masks = select_max_value_for_each_index(object_idx[..., 0], prob_map.contiguous())
    
    prob_map = prob_map[..., None]

    confidence = colors[..., 3, None]
    order = colors[..., 5, None]

    confidence_weight = prob_map * torch.exp((confidence - 1) * 10)
    confidence_weight_sum = confidence_weight.sum(dim=-2).clamp(min=eps)

    #order_weight = prob_map * torch.exp((confidence - 1) * 10 + (order - 1) * 10)  # * torch.exp((confidence - 1) * 10)
    #order_weight_sum = order_weight.sum(dim=-2).clamp(min=eps)
    # print(alpha_sum)

    # print(rgb.shape, alpha.shape, (rgb * alpha).shape, alpha_sum.shape)
    rgb_blended = (rgb * confidence_weight).sum(dim=-2) / confidence_weight_sum

    mask = confidence  # > 0.5 ).to(torch.float32)
