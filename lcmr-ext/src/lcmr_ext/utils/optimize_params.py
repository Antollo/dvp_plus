...
from lcmr.utils.elliptic_fourier_descriptors import normalize_efd
...
@typechecked
def optimize_params(
    scene: Union[SceneData, Callable[[], SceneData]],
    target: SceneData,
    renderer: Renderer2D,
    ...
    loss_func = loss_func.to(next(iter(params)).device)
    
    if isinstance(scene, SceneData):
        temp_scene = scene
        scene_fn = lambda: temp_scene
    else:
        scene_fn = scene
    ...
    for epoch in it:
        ...
        if scene.scene.layer.object.efd != None:
            with torch.no_grad():
                scene.scene.layer.object.efd[:] = normalize_efd(scene.scene.layer.object.efd)

        if show_progress:
            with torch.no_grad():
                it.set_description(f"loss: {loss.detach().cpu().item():.4f}")
                if display_interval > 0 and epoch % display_interval == 0:
                    img_grid = make_img_grid((pred.image_rgb_top, target.image_rgb_top), nrow=max(len(pred.image_rgb_top), len(target.image_rgb_top)))
